...
<!DOCTYPE html><html><head><meta charset="UTF-8"><title>품목정보 목록</title>
<style>
...
<div class="wrap">
  <div class="mod-nav">
    <a href="${pageContext.request.contextPath}/items_def/?mode=list" class="active">품목정보(법정)</a>
    <a href="${pageContext.request.contextPath}/items_detail/?mode=list">세부품목정보</a>
  </div>
  <h2>품목정보 관리 (법정품목명)</h2>
  ...
  <%-- ASP 원본 컬럼: No / 법정품목명(LI_LEGAL_NAME) / 규제법규 / 규제제도 / 고시 / 노출 / 등록일 --%>
  <table class="tbl">
    <thead>
      <tr>
        <th style="width:60px">No.</th>
        <th>법정 품목명</th>
        <th style="width:140px">규제법규</th>
        <th style="width:140px">규제제도</th>
        ...
        <c:otherwise>
          <c:forEach var="row" items="${list}" varStatus="st">
            <tr>
              ...
              <td><a href="?mode=form&li_idx=${row.li_idx}&page=${page}&qKey=${fn:escapeXml(qKey)}&qWord=${fn:escapeXml(qWord)}&qLL=${qLL}&qLR=${qLR}&qLN=${qLN}">${fn:escapeXml(row.li_legal_name != null ? row.li_legal_name : '')}</a></td>
              ...
              <td class="center">${row.li_is_use eq 'Y' ? '사용' : '미사용'}</td>
              <td class="center">${row.li_reg_date}</td>
            </tr>
          </c:forEach>
        </c:otherwise>
      </c:choose>
