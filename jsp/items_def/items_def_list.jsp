...
<title>품목정보 — DPL 법규정보 관리시스템</title>
...
<script>
function jfCreate(idx){location.href="?mode=form&li_idx="+idx+"&qLL=${qLL}&qLR=${qLR}&qLN=${qLN}&qKey=${fn:escapeXml(qKey)}&qWord=${fn:escapeXml(qWord)}&page=${page}";}
...
$(function(){
  ...
  $("#qLL").change(function(){
    var idx=$(this).val();$("#qLR option:gt(0),#qLN option:gt(0)").remove();
  ...
  $("#qLR").change(function(){
    var idx=$(this).val();$("#qLN option:gt(0)").remove();
    if(idx&&idx!="0") fetch("/ajax/?type=notify&qLR="+idx).then(r=>r.json()).then(d=>{(d.rows||[]).forEach(row=>{$("#qLN").append('<option value="'+row.ln_idx+'">'+(row.ln_title||row.ln_notify)+'</option>');});});
  });
});
</script>
</head><body>
...
<div id="container">
  <div class="title title_navi">
    <div class="title_text">품목정보</div>
    <p><span>Home</span><span>법규정보DB 관리</span><span>품목정보</span></p>
  </div>
        ...
        <tr>
          <th>규제법률</th><td>
            <select name="qLL" id="qLL">
              ...
              <c:forEach var="ll" items="${legalList}"><option value="${ll.ll_idx}" <c:if test="${ll.ll_idx==qLL}">selected</c:if>>${fn:escapeXml(ll.ll_title)}</option></c:forEach>
            </select>
            <select name="qLR" id="qLR">
              <option value="0">-- 선택 --</option>
              <c:forEach var="lr" items="${regulationList}"><option value="${lr.lr_idx}" <c:if test="${lr.lr_idx==qLR}">selected</c:if>>${fn:escapeXml(lr.lr_title)}</option></c:forEach>
            </select>
            <select name="qLN" id="qLN">
              <option value="0">-- 선택 --</option>
              <c:forEach var="ln" items="${notifyList}"><option value="${ln.ln_idx}" <c:if test="${ln.ln_idx==qLN}">selected</c:if>>${fn:escapeXml(ln.ln_title != null ? ln.ln_title : '')}</option></c:forEach>
            </select>
          </td>
        </tr>
        <tr>
          <th>검색어</th><td>
            <select name="qKey">
              ...
              <option value="TITLE" <c:if test="${qKey=='TITLE'}">selected</c:if>>법정품목명</option>
            </select>
  ...
  <div class="table_type_03"><table border="0" cellspacing="0" cellpadding="0">
    <colgroup><col style="width:80px"><col><col style="width:140px"><col style="width:140px"><col style="width:160px"><col style="width:70px"><col style="width:110px"></colgroup>
    <thead><tr>
      <th>No.</th>
      <th data-act="ls-sort" data-sort="TITLE" style="cursor:pointer">법정 품목명</th>
      ...
      <th data-act="ls-sort" data-sort="NOTIFY" style="cursor:pointer">정부고시(부속서)</th>
    ...
    <tbody>
      <c:choose>
        <c:when test="${empty list}"><tr><td colspan="7" style="text-align:center;padding:30px">검색된 내용이 없습니다.</td></tr></c:when>
        <c:otherwise>
          <c:forEach var="row" items="${list}" varStatus="st">
            <tr>
              <td style="text-align:center">${total-(page-1)*listSize-st.index}</td>
              <td class="_tit"><a href="javascript:jfCreate('${row.li_idx}');">${fn:escapeXml(row.li_legal_name != null ? row.li_legal_name : '')}</a></td>
              ...
              <td style="text-align:center">${fn:escapeXml(row.ln_title != null ? row.ln_title : (row.ln_notify != null ? row.ln_notify : ''))}</td>
              <td style="text-align:center">${row.li_is_use eq 'Y' ? '사용' : '미사용'}</td>
              <td style="text-align:center">${row.li_reg_date}</td>
            </tr>
          </c:forEach>
        </c:otherwise>
      </c:choose>
...
<div class="paginate_ui">
    <c:if test="${page>1}">
      <a class="pre" href="?mode=list&page=1&qLL=${qLL}&qLR=${qLR}&qLN=${qLN}&qKey=${fn:escapeXml(qKey)}&qWord=${fn:escapeXml(qWord)}&qSort=${fn:escapeXml(qSort)}"><img src="/static/img_admin/btn/btn_paginate_ui_prev_02.gif" alt="처음으로"></a>
      <a class="pre" href="?mode=list&page=${page-1}&qLL=${qLL}&qLR=${qLR}&qLN=${qLN}&qKey=${fn:escapeXml(qKey)}&qWord=${fn:escapeXml(qWord)}&qSort=${fn:escapeXml(qSort)}"><img src="/static/img_admin/btn/btn_paginate_ui_prev_01.gif" alt="이전"></a>
    </c:if>
    <c:forEach begin="1" end="${pageCnt}" var="p">
      <c:if test="${p>=page-4 && p<=page+4}">
        <c:choose>
          <c:when test="${p==page}"><strong>${p}</strong></c:when>
          <c:otherwise><a href="?mode=list&page=${p}&qLL=${qLL}&qLR=${qLR}&qLN=${qLN}&qKey=${fn:escapeXml(qKey)}&qWord=${fn:escapeXml(qWord)}&qSort=${fn:escapeXml(qSort)}">${p}</a></c:otherwise>
        </c:choose>
      </c:if>
    </c:forEach>
    <c:if test="${page<pageCnt}">
      <a class="next" href="?mode=list&page=${page+1}&qLL=${qLL}&qLR=${qLR}&qLN=${qLN}&qKey=${fn:escapeXml(qKey)}&qWord=${fn:escapeXml(qWord)}&qSort=${fn:escapeXml(qSort)}"><img src="/static/img_admin/btn/btn_paginate_ui_next_01.gif" alt="다음"></a>
      <a class="next" href="?mode=list&page=${pageCnt}&qLL=${qLL}&qLR=${qLR}&qLN=${qLN}&qKey=${fn:escapeXml(qKey)}&qWord=${fn:escapeXml(qWord)}&qSort=${fn:escapeXml(qSort)}"><img src="/static/img_admin/btn/btn_paginate_ui_next_02.gif" alt="마지막으로"></a>
    </c:if>
  </div>
</div>
<div id="footer">
