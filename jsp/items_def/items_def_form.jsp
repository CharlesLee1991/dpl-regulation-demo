...
<script>
function jfSubmit(){
  if(!$("#li_legal_name").val().trim()){alert("법정 품목명을 입력해주세요.");return;}
  ...
  if(!$("input[name='li_is_use']:checked").val()){alert("사용여부를 선택해주세요.");return;}
  $("#frmInfo").submit();
}
...
$(function(){
  $("#ll_idx").change(function(){
    var idx=$(this).val();$("#lr_idx option:gt(0),#ln_idx option:gt(0)").remove();
  ...
  $("#lr_idx").change(function(){
    var idx=$(this).val();$("#ln_idx option:gt(0)").remove();
...
<div id="container">
  ...
  <form id="frmInfo" name="frmInfo" method="post" action="?mode=proc">
    ...
    <input type="hidden" name="li_idx" value="${liIdx}">
    ...
    <div class="table_type_01">
      <table border="0" cellspacing="0" cellpadding="0">
        ...
        <tbody>
          <tr>
            <th><i class="import_i">*</i>법정 품목명</th>
            <td><input type="text" name="li_legal_name" id="li_legal_name" class="inp _w_full"
                value="${fn:escapeXml(info.li_legal_name != null ? info.li_legal_name : '')}" maxlength="200"></td>
          </tr>
          <tr>
          ...
          <tr>
            <th>적용범위</th>
            <td><textarea name="li_scope" class="inp _w_full" rows="4" style="resize:vertical">${fn:escapeXml(info.li_scope != null ? info.li_scope : '')}</textarea></td>
          </tr>
          <tr>
            <th>제외대상</th>
            <td><textarea name="li_exception" class="inp _w_full" rows="4" style="resize:vertical">${fn:escapeXml(info.li_exception != null ? info.li_exception : '')}</textarea></td>
          </tr>
          <tr>
            <th><i class="import_i">*</i>사용여부</th>
            <td>
              <input type="radio" name="li_is_use" id="li_is_use_y" value="Y" <c:if test="${empty info.li_is_use || info.li_is_use eq 'Y'}">checked</c:if>>
              <label for="li_is_use_y" class="radio_label">사용</label>
              <input type="radio" name="li_is_use" id="li_is_use_n" value="N" <c:if test="${info.li_is_use eq 'N'}">checked</c:if>>
              <label for="li_is_use_n" class="radio_label">미사용</label>
            </td>
          </tr>
          <c:if test="${action eq 'MOD'}"><tr><th>등록일</th><td>${info.li_reg_date}</td></tr></c:if>
        </tbody>
      </table>
    </div>
