...
<!DOCTYPE html><html><head><meta charset="UTF-8"><title>품목정보 ${action eq 'ADD' ? '등록' : '수정'}</title>
<style>
  ...
  .tbl-form input[type=text]{width:98%}.tbl-form textarea{width:98%;height:120px;resize:vertical}
...
<div class="wrap">
  <h2>품목정보 (법정품목명) ${action eq 'ADD' ? '등록' : '수정'}</h2>
  <form id="frmInfo" method="post" action="?mode=proc" onsubmit="return jfSubmit()">
    ...
    <table class="tbl-form">
      <tr>
        <th><i class="req">*</i> 법정 품목명</th>
        <td><input type="text" id="li_legal_name" name="li_legal_name" value="${fn:escapeXml(info.li_legal_name != null ? info.li_legal_name : '')}" maxlength="100"></td>
      </tr>
      <%-- ASP 원본: "규제법률/사항/고시" 3단계 --%>
      <tr>
      ...
      <tr>
        <th>적용범위</th>
        <td><textarea name="li_scope">${fn:escapeXml(info.li_scope != null ? info.li_scope : '')}</textarea></td>
      </tr>
      <tr>
        <th>제외대상</th>
        <td><textarea name="li_exception">${fn:escapeXml(info.li_exception != null ? info.li_exception : '')}</textarea></td>
      </tr>
      <tr>
        <th><i class="req">*</i> 사용여부</th>
        <td class="radio-group">
          <label><input type="radio" name="li_is_use" value="Y" <c:if test="${empty info.li_is_use || info.li_is_use eq 'Y'}">checked</c:if>> 사용</label>
          <label><input type="radio" name="li_is_use" value="N" <c:if test="${info.li_is_use eq 'N'}">checked</c:if>> 미사용</label>
        </td>
      </tr>
      <c:if test="${action eq 'MOD'}">
        <tr><th>등록일</th><td>${info.li_reg_date}</td></tr>
      </c:if>
    </table>
    <div class="btn-wrap">
...
function loadLN(lrIdx){
  var $ln=document.getElementById('ln_idx'); while($ln.options.length>1)$ln.remove(1);
...
function jfSubmit(){
  if(!document.getElementById('li_legal_name').value.trim()){alert('법정 품목명을 입력해주세요');return false;}
  ...
  if(!document.querySelector('input[name="li_is_use"]:checked')){alert('사용여부를 선택해주세요');return false;}
  return true;
}
</script>
</body></html>
