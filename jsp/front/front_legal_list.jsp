...
<div id="container">
  ...
  <div id="contents">
    ...
    <div class="result_header">
      <span class="total"><em>${total}</em>건의 검색결과가 있습니다.</span>
    </div>

    <ul class="result_list">
      <c:choose>
        <c:when test="${empty list}">
          <li class="no_result"><p>검색된 결과가 없습니다.</p></li>
        </c:when>
        <c:otherwise>
            ...
            <li>
              <a href="/front/legal/view?lr_idx=${row.lr_idx}">
                <div class="label_wrap">
                  <span class="label_cate">${fn:escapeXml(row.ll_title != null ? row.ll_title : '')}</span>
                </div>
                <p class="tit">${fn:escapeXml(row.lr_title)}</p>
                <div class="info_wrap">
                  <span class="date">${row.lr_reg_date}</span>
                </div>
              </a>
            </li>
          </c:forEach>
    ...
    <div class="paging">
      <c:if test="${page>1}">
        <a class="first" href="?page=1&qLL=${qLL}&qLR=${qLR}&qWord=${fn:escapeXml(qWord)}">처음</a>
        <a class="prev" href="?page=${page-1}&qLL=${qLL}&qLR=${qLR}&qWord=${fn:escapeXml(qWord)}">이전</a>
      </c:if>
      <c:forEach begin="1" end="${pageCnt}" var="p">
        <c:if test="${p>=page-4 && p<=page+4}">
          <c:choose>
            <c:when test="${p==page}"><strong>${p}</strong></c:when>
            <c:otherwise><a href="?page=${p}&qLL=${qLL}&qLR=${qLR}&qWord=${fn:escapeXml(qWord)}">${p}</a></c:otherwise>
          </c:choose>
        </c:if>
      </c:forEach>
      <c:if test="${page<pageCnt}">
        <a class="next" href="?page=${page+1}&qLL=${qLL}&qLR=${qLR}&qWord=${fn:escapeXml(qWord)}">다음</a>
        <a class="last" href="?page=${pageCnt}&qLL=${qLL}&qLR=${qLR}&qWord=${fn:escapeXml(qWord)}">마지막</a>
      </c:if>
    </div>
  </div><%-- /contents --%>
</div><%-- /container --%>
<script>
function jfReset(){
  $("#qWord").val(""); $("#qLL, #qLR").val(""); $("#frmSearch").submit();
}
$(function(){
  $("#qLL").change(function(){
