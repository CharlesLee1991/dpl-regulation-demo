...
<script>
function jfCreate(idx){location.href="?mode=form&lr_idx="+idx+"&qLL=${qLL}&qKey=${fn:escapeXml(qKey)}&qWord=${fn:escapeXml(qWord)}&page=${page}";}
...
<div id="container">
  ...
  <div class="table_type_03"><table border="0" cellspacing="0" cellpadding="0">
    ...
    <thead><tr>
      <th>No.</th>
      <th data-act="ls-sort" data-sort="TITLE" style="cursor:pointer">관리제도</th>
        ...
        <c:otherwise>
          <c:forEach var="row" items="${list}" varStatus="st">
            <tr>
              ...
              <td class="_tit"><a href="javascript:jfCreate('${row.lr_idx}');">${fn:escapeXml(row.lr_title)}</a></td>
...
<div class="paginate_ui">
    <c:if test="${page>1}">
      <a class="pre" href="?mode=list&page=1&qLL=${qLL}&qKey=${fn:escapeXml(qKey)}&qWord=${fn:escapeXml(qWord)}&qSort=${fn:escapeXml(qSort)}"><img src="/static/img_admin/btn/btn_paginate_ui_prev_02.gif" alt="처음으로"></a>
      <a class="pre" href="?mode=list&page=${page-1}&qLL=${qLL}&qKey=${fn:escapeXml(qKey)}&qWord=${fn:escapeXml(qWord)}&qSort=${fn:escapeXml(qSort)}"><img src="/static/img_admin/btn/btn_paginate_ui_prev_01.gif" alt="이전"></a>
    </c:if>
    <c:forEach begin="1" end="${pageCnt}" var="p">
      <c:if test="${p>=page-4 && p<=page+4}">
        <c:choose>
          <c:when test="${p==page}"><strong>${p}</strong></c:when>
          <c:otherwise><a href="?mode=list&page=${p}&qLL=${qLL}&qKey=${fn:escapeXml(qKey)}&qWord=${fn:escapeXml(qWord)}&qSort=${fn:escapeXml(qSort)}">${p}</a></c:otherwise>
        </c:choose>
      </c:if>
    </c:forEach>
    <c:if test="${page<pageCnt}">
      <a class="next" href="?mode=list&page=${page+1}&qLL=${qLL}&qKey=${fn:escapeXml(qKey)}&qWord=${fn:escapeXml(qWord)}&qSort=${fn:escapeXml(qSort)}"><img src="/static/img_admin/btn/btn_paginate_ui_next_01.gif" alt="다음"></a>
      <a class="next" href="?mode=list&page=${pageCnt}&qLL=${qLL}&qKey=${fn:escapeXml(qKey)}&qWord=${fn:escapeXml(qWord)}&qSort=${fn:escapeXml(qSort)}"><img src="/static/img_admin/btn/btn_paginate_ui_next_02.gif" alt="마지막으로"></a>
    </c:if>
  </div>
</div>
<div id="footer">
