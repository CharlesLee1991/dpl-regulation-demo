  ...
  <c:choose>
    ...
    <c:otherwise>
      <table class="tbl-view">
        <tr><th>규제사항명</th><td>${fn:escapeXml(info.lr_title)}</td></tr>
        <tr><th>규제법률 IDX</th><td>${info.ll_idx}</td></tr>
        <tr><th>적용조건</th><td>${fn:escapeXml(info.lr_condition)}</td></tr>
        <tr><th>인증 가이드</th><td>${fn:escapeXml(info.lr_certify_guide)}</td></tr>
        <tr><th>벌칙/과태료</th><td>${fn:escapeXml(info.lr_penalty)}</td></tr>
        <tr><th>사용여부</th><td>${info.lr_is_use eq 'Y' ? '사용' : '미사용'}</td></tr>
        <tr><th>등록일</th><td>${info.lr_reg_date} (${info.lr_reg_user})</td></tr>
        <tr><th>수정일</th><td>${info.lr_upd_date} (${info.lr_upd_user})</td></tr>
      </table>
    </c:otherwise>
  </c:choose>
  <div class="btn-wrap">
    <button class="btn btn-primary"
      onclick="location.href='?mode=form&lr_idx=${lrIdx}'">수정</button>
