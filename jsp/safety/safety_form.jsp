...
function jfSubmit(){
  ...
  if(!$("#ls_title").val().trim()){alert("안전요건을 입력해주세요.");$("#ls_title").focus();return;}
  if(!$("#ls_content").val().trim()){alert("상세내용을 입력해주세요.");$("#ls_content").focus();return;}
  if(!$("input[name='ls_is_use']:checked").val()){alert("사용여부를 선택해주세요.");return;}
  $("#frmInfo").submit();
}
    ...
    <div class="table_type_01">
      <table border="0" cellspacing="0" cellpadding="0">
        ...
        <tbody>
          ...
          <tr>
            <th><i class="import_i">*</i>안전요건</th>
            <td><input type="text" name="ls_title" id="ls_title" class="inp _w_full"
                value="${fn:escapeXml(info.ls_title != null ? info.ls_title : '')}" maxlength="200"></td>
          </tr>
          <tr>
            <th><i class="import_i">*</i>상세내용</th>
            <td><textarea name="ls_content" id="ls_content" class="inp _w_full" rows="8"
                style="resize:vertical">${fn:escapeXml(info.ls_content != null ? info.ls_content : '')}</textarea></td>
          </tr>
          <tr>
            <th><i class="import_i">*</i>사용여부</th>
            <td>
              <input type="radio" name="ls_is_use" id="ls_is_use_y" value="Y" <c:if test="${empty info.ls_is_use || info.ls_is_use eq 'Y'}">checked</c:if>>
              <label for="ls_is_use_y" class="radio_label">사용</label>
              <input type="radio" name="ls_is_use" id="ls_is_use_n" value="N" <c:if test="${info.ls_is_use eq 'N'}">checked</c:if>>
              <label for="ls_is_use_n" class="radio_label">미사용</label>
            </td>
          </tr>
          <c:if test="${action eq 'MOD'}">
            <tr><th>등록일</th><td>${info.ls_reg_date}</td></tr>
          </c:if>
        </tbody>
      </table>
    </div>
