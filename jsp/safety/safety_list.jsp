...
<title>안전요건 — DPL 법규정보 관리시스템</title>
...
<script>
function jfCreate(idx){location.href="?mode=form&ls_idx="+idx+"&qLL=${qLL}&qLR=${qLR}&qLN=${qLN}&qKey=${fn:escapeXml(qKey)}&qWord=${fn:escapeXml(qWord)}&page=${page}";}
...
<div id="container">
  <div class="title title_navi">
    <div class="title_text">안전요건</div>
    <p><span>Home</span><span>법규정보DB 관리</span><span>안전요건</span></p>
  </div>
  ...
  <div class="table_type_03"><table border="0" cellspacing="0" cellpadding="0">
    ...
    <thead><tr>
      <th>No.</th>
      <th data-act="ls-sort" data-sort="TITLE" style="cursor:pointer">안전요건</th>
        ...
        <c:otherwise>
          <c:forEach var="row" items="${list}" varStatus="st">
            <tr>
              ...
              <td class="_tit"><a href="javascript:jfCreate('${row.ls_idx}');">${fn:escapeXml(row.ls_title != null ? row.ls_title : '')}</a></td>
              ...
              <td style="text-align:center">${fn:escapeXml(row.ln_title != null ? row.ln_title : '')}</td>
              <td style="text-align:center">${row.ls_is_use eq 'Y' ? '사용' : '미사용'}</td>
              <td style="text-align:center">${row.ls_reg_date}</td>
            </tr>
          </c:forEach>
        </c:otherwise>
      </c:choose>
