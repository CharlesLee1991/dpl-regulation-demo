...
<!DOCTYPE html><html><head><meta charset="UTF-8"><title>세부품목정보 목록</title>
<style>
  body{font-family:'Malgun Gothic',sans-serif;font-size:13px}
  .wrap{max-width:1300px;margin:20px auto;padding:0 15px}
  ...
  .mod-nav{display:flex;gap:10px;margin-bottom:12px}
  .mod-nav a{padding:6px 16px;background:#e8edf5;border-radius:4px;text-decoration:none;color:#333;font-size:13px}
  .mod-nav a.active{background:#3d6fd4;color:#fff}
</style>
</head><body>
<div class="wrap">
  <div class="mod-nav">
    <a href="${pageContext.request.contextPath}/items_def/?mode=list">품목정보(법정)</a>
    <a href="${pageContext.request.contextPath}/items_detail/?mode=list" class="active">세부품목정보</a>
  </div>
  <h2>세부품목정보 관리 (일반품목명)</h2>
  <form id="frmSearch" method="get" action="">
    ...
    <div class="search-bar">
      ...
      <input type="text" name="qWord" value="${fn:escapeXml(qWord)}" placeholder="검색어" style="width:160px">
  ...
  <%-- ASP 원본 컬럼: No / 일반품목명 / 사용연령(체중) / 재질(형태) / 규제법규 / 규제제도 / 정부고시 / 노출 --%>
  <table class="tbl">
    <thead>
      <tr>
        <th style="width:55px">No.</th>
        <th>일반 품목명</th>
        <th style="width:100px">사용연령(체중)</th>
        <th style="width:100px">재질(형태)</th>
        <th style="width:130px">규제법규</th>
        <th style="width:130px">규제제도</th>
        <th style="width:160px">정부고시(부속서)</th>
    ...
    <tbody>
      <c:choose>
        <c:when test="${empty list}">
          <tr><td colspan="8" class="center" style="padding:30px;color:#999">검색된 내용이 없습니다.</td></tr>
        </c:when>
        <c:otherwise>
          <c:forEach var="row" items="${list}" varStatus="st">
            <tr>
              <td class="center">${total-(page-1)*listSize-st.index}</td>
              <td><a href="?mode=form&ld_idx=${row.ld_idx}&page=${page}&qLL=${qLL}&qLR=${qLR}&qLN=${qLN}">${fn:escapeXml(row.ld_item_name != null ? row.ld_item_name : '')}</a></td>
              <td class="center">${fn:escapeXml(row.ld_use_age != null ? row.ld_use_age : '')}</td>
              <td class="center">${fn:escapeXml(row.ld_material != null ? row.ld_material : '')}</td>
              ...
              <td class="center">${row.ld_is_use eq 'Y' ? '사용' : '미사용'}</td>
            </tr>
          </c:forEach>
        </c:otherwise>
      </c:choose>
  ...
  <div class="paging">
    <c:if test="${page>1}">
      <a href="?mode=list&page=1&qLL=${qLL}&qLR=${qLR}&qLN=${qLN}">◀◀</a>
      <a href="?mode=list&page=${page-1}&qLL=${qLL}&qLR=${qLR}&qLN=${qLN}">◀</a>
    </c:if>
    <c:forEach begin="1" end="${pageCnt}" var="p">
      <c:if test="${p>=page-4 && p<=page+4}">
        <a href="?mode=list&page=${p}&qLL=${qLL}&qLR=${qLR}&qLN=${qLN}" class="${p==page?'cur':''}">${p}</a>
      </c:if>
    </c:forEach>
    <c:if test="${page<pageCnt}">
      <a href="?mode=list&page=${page+1}&qLL=${qLL}&qLR=${qLR}&qLN=${qLN}">▶</a>
      <a href="?mode=list&page=${pageCnt}&qLL=${qLL}&qLR=${qLR}&qLN=${qLN}">▶▶</a>
    </c:if>
  </div>
</div>
<script>
