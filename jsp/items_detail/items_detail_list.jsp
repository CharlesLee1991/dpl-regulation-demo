...
<script>
function jfCreate(idx){location.href="?mode=form&ld_idx="+idx+"&qLL=${qLL}&qLR=${qLR}&qLN=${qLN}&page=${page}";}
...
<div id="container">
  ...
  <div class="table_type_03"><table border="0" cellspacing="0" cellpadding="0">
    <colgroup><col><col><col style="width:100px"><col style="width:100px"><col style="width:130px"><col style="width:130px"><col style="width:160px"><col style="width:70px"></colgroup>
    <thead><tr>
      <th>No.</th>
      <th data-act="ls-sort" data-sort="TITLE" style="cursor:pointer">일반 품목명</th>
      <th data-act="ls-sort" data-sort="AGE" style="cursor:pointer">사용연령(체중)</th>
      <th data-act="ls-sort" data-sort="MAT" style="cursor:pointer">재질(형태)</th>
    ...
    <tbody>
      <c:choose>
        <c:when test="${empty list}"><tr><td colspan="8" style="text-align:center;padding:30px">검색된 내용이 없습니다.</td></tr></c:when>
        <c:otherwise>
          <c:forEach var="row" items="${list}" varStatus="st">
            <tr>
              <td style="text-align:center">${total-(page-1)*listSize-st.index}</td>
              <td class="_tit"><a href="javascript:jfCreate('${row.ld_idx}');">${fn:escapeXml(row.ld_item_name != null ? row.ld_item_name : '')}</a></td>
              <td style="text-align:center">${fn:escapeXml(row.ld_use_age != null ? row.ld_use_age : '')}</td>
              <td style="text-align:center">${fn:escapeXml(row.ld_material != null ? row.ld_material : '')}</td>
              ...
              <td style="text-align:center">${row.ld_is_use eq 'Y' ? '사용' : '미사용'}</td>
            </tr>
          </c:forEach>
        </c:otherwise>
      </c:choose>
...
<div class="paginate_ui">
    <c:if test="${page>1}">
      <a class="pre" href="?mode=list&page=1&qLL=${qLL}&qLR=${qLR}&qLN=${qLN}&qWord=${fn:escapeXml(qWord)}&qSort=${fn:escapeXml(qSort)}"><img src="/static/img_admin/btn/btn_paginate_ui_prev_02.gif" alt="처음으로"></a>
      <a class="pre" href="?mode=list&page=${page-1}&qLL=${qLL}&qLR=${qLR}&qLN=${qLN}&qWord=${fn:escapeXml(qWord)}&qSort=${fn:escapeXml(qSort)}"><img src="/static/img_admin/btn/btn_paginate_ui_prev_01.gif" alt="이전"></a>
    </c:if>
    <c:forEach begin="1" end="${pageCnt}" var="p">
      <c:if test="${p>=page-4 && p<=page+4}">
        <c:choose>
          <c:when test="${p==page}"><strong>${p}</strong></c:when>
          <c:otherwise><a href="?mode=list&page=${p}&qLL=${qLL}&qLR=${qLR}&qLN=${qLN}&qWord=${fn:escapeXml(qWord)}&qSort=${fn:escapeXml(qSort)}">${p}</a></c:otherwise>
        </c:choose>
      </c:if>
    </c:forEach>
    <c:if test="${page<pageCnt}">
      <a class="next" href="?mode=list&page=${page+1}&qLL=${qLL}&qLR=${qLR}&qLN=${qLN}&qWord=${fn:escapeXml(qWord)}&qSort=${fn:escapeXml(qSort)}"><img src="/static/img_admin/btn/btn_paginate_ui_next_01.gif" alt="다음"></a>
      <a class="next" href="?mode=list&page=${pageCnt}&qLL=${qLL}&qLR=${qLR}&qLN=${qLN}&qWord=${fn:escapeXml(qWord)}&qSort=${fn:escapeXml(qSort)}"><img src="/static/img_admin/btn/btn_paginate_ui_next_02.gif" alt="마지막으로"></a>
    </c:if>
  </div>
</div>
<div id="footer">
