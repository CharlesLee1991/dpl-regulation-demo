...
<script>
function jfCreate(idx){location.href="?mode=form&ll_idx="+idx+"&qKey=${fn:escapeXml(qKey)}&qWord=${fn:escapeXml(qWord)}&page=${page}";}
...
<div id="container">
  ...
  <form name="frmSearch" id="frmSearch" method="get" action="?mode=list" onsubmit="return jfSearch()">
    ...
    <div class="table_type_02"><table border="0" cellspacing="0" cellpadding="0">
      <colgroup><col width="118"><col width="*"><col width="118"><col width="*"></colgroup>
      <tbody><tr>
        <th>검색어</th><td colspan="3">
          <select name="qKey" id="qKey">
            <option value="" <c:if test="${qKey==''}">selected</c:if>>전체</option>
            <option value="TITLE" <c:if test="${qKey=='TITLE'}">selected</c:if>>법규명</option>
            <option value="DEPT" <c:if test="${qKey=='DEPT'}">selected</c:if>>관리부처</option>
          </select>
          <input type="text" name="qWord" id="qWord" class="inp" value="${fn:escapeXml(qWord)}">
  ...
  <div class="table_type_03"><table border="0" cellspacing="0" cellpadding="0">
    <colgroup><col style="width:80px"><col><col style="width:200px"><col style="width:80px"><col style="width:120px"></colgroup>
    <thead><tr>
      <th>No.</th>
      <th data-act="ls-sort" data-sort="TITLE" style="cursor:pointer">법규명</th>
      <th data-act="ls-sort" data-sort="DEPT" style="cursor:pointer">관리부처</th>
    ...
    <tbody>
      <c:choose>
        <c:when test="${empty list}"><tr><td colspan="5" style="text-align:center;padding:30px">검색된 내용이 없습니다.</td></tr></c:when>
        <c:otherwise>
          <c:forEach var="row" items="${list}" varStatus="st">
            <tr>
              <td style="text-align:center">${total-(page-1)*listSize-st.index}</td>
              <td class="_tit"><a href="javascript:jfCreate('${row.ll_idx}');">${fn:escapeXml(row.ll_title)}</a></td>
              <td style="text-align:center">${fn:escapeXml(row.ll_dept != null ? row.ll_dept : '')}</td>
              <td style="text-align:center">${row.ll_is_use eq 'Y' ? '사용' : '미사용'}</td>
              <td style="text-align:center">${row.ll_reg_date}</td>
            </tr>
          </c:forEach>
        </c:otherwise>
      </c:choose>
...
<div class="paginate_ui">
    <c:if test="${page>1}">
      <a class="pre" href="?mode=list&page=1&qKey=${fn:escapeXml(qKey)}&qWord=${fn:escapeXml(qWord)}&qSort=${fn:escapeXml(qSort)}"><img src="/static/img_admin/btn/btn_paginate_ui_prev_02.gif" alt="처음으로"></a>
      <a class="pre" href="?mode=list&page=${page-1}&qKey=${fn:escapeXml(qKey)}&qWord=${fn:escapeXml(qWord)}&qSort=${fn:escapeXml(qSort)}"><img src="/static/img_admin/btn/btn_paginate_ui_prev_01.gif" alt="이전"></a>
    </c:if>
    <c:forEach begin="1" end="${pageCnt}" var="p">
      <c:if test="${p>=page-4 && p<=page+4}">
        <c:choose>
          <c:when test="${p==page}"><strong>${p}</strong></c:when>
          <c:otherwise><a href="?mode=list&page=${p}&qKey=${fn:escapeXml(qKey)}&qWord=${fn:escapeXml(qWord)}&qSort=${fn:escapeXml(qSort)}">${p}</a></c:otherwise>
        </c:choose>
      </c:if>
    </c:forEach>
    <c:if test="${page<pageCnt}">
      <a class="next" href="?mode=list&page=${page+1}&qKey=${fn:escapeXml(qKey)}&qWord=${fn:escapeXml(qWord)}&qSort=${fn:escapeXml(qSort)}"><img src="/static/img_admin/btn/btn_paginate_ui_next_01.gif" alt="다음"></a>
      <a class="next" href="?mode=list&page=${pageCnt}&qKey=${fn:escapeXml(qKey)}&qWord=${fn:escapeXml(qWord)}&qSort=${fn:escapeXml(qSort)}"><img src="/static/img_admin/btn/btn_paginate_ui_next_02.gif" alt="마지막으로"></a>
    </c:if>
  </div>
</div>
<div id="footer">
