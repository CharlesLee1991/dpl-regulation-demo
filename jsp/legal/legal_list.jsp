...
<div class="wrap">
  <h2>규제법률 관리</h2>
  <%-- 상위 모듈 링크 (ASP 원본: breadcrumb) --%>
  <div class="nav-link">▶ <a href="${pageContext.request.contextPath}/regulation/?mode=list">규제사항 목록</a></div>

  <%-- 검색 폼 (ASP 원본: qKey=TITLE|DEPT + qWord) --%>
  ...
  <%-- 목록 (ASP 원본: No / 법규명(LL_TITLE) / 관리부처(LL_DEPT) / 노출(LL_IS_USE) / 등록일) --%>
        ...
        <c:otherwise>
          <c:forEach var="row" items="${list}" varStatus="st">
            <tr>
              ...
              <td>
                <a href="?mode=form&ll_idx=${row.ll_idx}&page=${page}&qKey=${fn:escapeXml(qKey)}&qWord=${fn:escapeXml(qWord)}">
                  ${fn:escapeXml(row.ll_title)}
                </a>
              </td>
              <td class="center">${fn:escapeXml(row.ll_dept)}</td>
              <td class="center">${row.ll_is_use eq 'Y' ? '사용' : '미사용'}</td>
              <td class="center">${row.ll_reg_date}</td>
            </tr>
          </c:forEach>
        </c:otherwise>
      </c:choose>
