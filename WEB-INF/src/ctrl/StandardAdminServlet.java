 ...
 * 구분 ST_DIV(공통/품질기준/인스펙션), 기준서번호 ST_CODE, 최신개정일 ST_VER_DATE, 적용품목 ST_ITEMS
 */
public class StandardAdminServlet extends HttpServlet {
    ...
    private void doList(HttpServletRequest req, HttpServletResponse resp) throws Exception {
        ...
        if (!qWord.isEmpty()) {
            ...
            if      ("CODE".equals(qKey))  w += " AND ISNULL(ST_CODE,'') LIKE N'%"+sw+"%'";
            else if ("TITLE".equals(qKey)) w += " AND ST_TITLE LIKE N'%"+sw+"%'";
            else if ("ITEMS".equals(qKey)) w += " AND ISNULL(ST_ITEMS,'') LIKE N'%"+sw+"%'";
            else w += " AND (ST_TITLE LIKE N'%"+sw+"%' OR ISNULL(ST_CODE,'') LIKE N'%"+sw+"%' OR ISNULL(ST_ITEMS,'') LIKE N'%"+sw+"%')";
        }
        List<Map<String,Object>> list = sql("SELECT ST_IDX,ISNULL(ST_DIV,'') AS ST_DIV,ISNULL(ST_CODE,'') AS ST_CODE,"
            + "ST_TITLE,ISNULL(ST_ITEMS,'') AS ST_ITEMS,ISNULL(ST_VER_DATE,'') AS ST_VER_DATE,ST_IS_USE,"
            + "CONVERT(NVARCHAR(10),ST_REG_DATE,120) AS ST_REG_DATE "
            + "FROM dpl_standard "+w+" ORDER BY ST_IDX DESC OFFSET "+offset+" ROWS FETCH NEXT "+PS+" ROWS ONLY");
    ...
    private void doForm(HttpServletRequest req, HttpServletResponse resp) throws Exception {
        ...
        if (idx>0) {
            List<Map<String,Object>> rows = sql("SELECT ST_IDX,ISNULL(ST_DIV,'') AS ST_DIV,ISNULL(ST_CODE,'') AS ST_CODE,"
                + "ST_TITLE,ISNULL(ST_ITEMS,'') AS ST_ITEMS,ISNULL(ST_VER_DATE,'') AS ST_VER_DATE,"
                + "ISNULL(ST_CONTENT,'') AS ST_CONTENT,ST_IS_USE "
                + "FROM dpl_standard WHERE ST_IDX="+idx);
            ...
            try (Connection conn = DBPool.getConnection()) {
                if ("ADD".equals(action)) {
                    try (PreparedStatement ps = conn.prepareStatement(
                        "INSERT INTO dpl_standard (ST_DIV,ST_CODE,ST_TITLE,ST_ITEMS,ST_VER_DATE,ST_CONTENT,ST_CATE,ST_IS_USE,ST_REG_DATE) "
                        + "VALUES (?,?,?,?,?,?,0,?,GETDATE())")) {
                        ps.setString(1,div); ps.setString(2,code); ps.setString(3,title); ps.setString(4,items);
                        ps.setString(5,ver); ps.setString(6,cont); ps.setString(7,isUse);
                        ps.executeUpdate();
                    }
                } else if ("MOD".equals(action) && idx>0) {
                    try (PreparedStatement ps = conn.prepareStatement(
                        "UPDATE dpl_standard SET ST_DIV=?,ST_CODE=?,ST_TITLE=?,ST_ITEMS=?,ST_VER_DATE=?,ST_CONTENT=?,ST_IS_USE=? WHERE ST_IDX=?")) {
                        ps.setString(1,div); ps.setString(2,code); ps.setString(3,title); ps.setString(4,items);
                        ps.setString(5,ver); ps.setString(6,cont); ps.setString(7,isUse); ps.setInt(8,idx);
                        ps.executeUpdate();
                    }
                } else if ("DEL".equals(action) && idx>0) {
                    try (PreparedStatement ps = conn.prepareStatement("DELETE FROM dpl_standard WHERE ST_IDX=?")) {
